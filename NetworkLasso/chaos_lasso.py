...
def solveX(data):
	...
	mu = data[data.size-5]
	x = data[0:inputs]
	a = data[inputs:int (inputs + sizeData)]
	neighs = data[int (inputs + sizeData):int (data.size-5)]
...
def main():
	...
	file = open("Chaos_Data_normalized.csv", "rU")
	...
	for line in file:
		G1.AddNode(counter)
		temp = TFltPr(float(line.split(",")[3]),float(line.split(",")[4]))
		locations.AddDat(counter, temp)
		tempData = TFltV()
		tempData.Add(float(line.split(",")[1]))
		tempData.Add(float(line.split(",")[2]))

		tempData.Add(float(line.split(",")[5])*10) #12 for normalized; 9 for raw
	...
	for NI in G1.Nodes():
		...
		for NI2 in G1.Nodes():
			if(NI.GetId() != NI2.GetId()):
				...
				#can we make the distances on poisson point process
	...
	while(lamb <= thresh or lamb == 0):
		...
		for i in testList:
			...
			prediction = xpred[0]*float(regressors[0]) + xpred[1]*float(regressors[1]) + xpred[2]
	...
	if(thresh > 0):
		...
		plt.ylabel('MSE')
		if(useConvex == 1):
			plt.savefig('image_chaos_convex',bbox_inches='tight')
		else:
			plt.savefig('image_chaos_nonconvex',bbox_inches='tight')
		...
		if(useConvex == 1):
			plt.savefig('consensus_chaos_convex',bbox_inches='tight')
			np.savetxt('text_chaos_convex.out', (pl1, pl2, pl3), delimiter=',', fmt='%1.4f')
		else:
			plt.savefig('consensus_chaos_nonconvex',bbox_inches='tight')
			np.savetxt('text_chaos_nonconvex.out', (pl1, pl2, pl3), delimiter=',', fmt='%1.4f')
